...
LoggedInUser loggedInUser;  // Deklaration von loggedInUser hinzugefügt
...
gboolean move_arrow_up(gpointer user_data);
...
void start_game(GtkWidget* widget, gpointer user_data) {
    ...
    g_timeout_add(250, (GSourceFunc)move_symbols, NULL);
...
gboolean on_draw(GtkWidget* widget, cairo_t* cr, gpointer user_data) {
    ...
    if (moving_arrow) {
        cairo_move_to(cr, arrow_col * cell_size, arrow_row * cell_size);
        cairo_show_text(cr, &arrowSymbol);
    }
...
gboolean move_arrow_up(gpointer user_data) {
    moving_arrow = TRUE;
    g_timeout_add(250, (GSourceFunc)move_arrow_up, NULL);
